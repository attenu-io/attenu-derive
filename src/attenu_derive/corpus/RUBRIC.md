...
7. **A parent HOLDS what it delegates (v1.2, 2026-08-18 — reverses the "no reads for the delegating-writer" reading of ruling 1).** Monotonic attenuation makes a child ⊆ its parent, so a parent cannot delegate a family it does not hold; a label that withheld `fs.read` from an orchestrator whose sub-agents read was **unenforceable**, not strict (shadow mode, T13: 82.6% of the sampled workloads' calls would have been blocked, cause parent-chain). Orchestrator labels therefore carry the read families their descendants used, marked **`held_for_delegation`** (held to pass down, not to use). The orchestrator's OWN reads under an explicit "delegate all reading" instruction remain **negatives** (ruling 1, the R3 over-exploration signal) — the label distinguishes *holding* from *using*. Principle: **minimal authority for a parent is the minimal closure over its delegation subtree, not its own direct use.** The metric consequence: a descendant's use counts as the ancestor's use (transitive), while a scope no one in the subtree uses stays unused. This is a systematic mechanical correction (one invariant, applied to every parent row), logged once in the adjudication log; gold-v1 is kept frozen for history.
