...
`Authority`**, always granted as `parent.meet(proposal)` by `delegation-guard`, so error can only
...
- `src/attenu_derive/sample/`   `attenu-sample` observe-mode recorder over the delegation-guard adapters (redacts at capture — ADR-05)
